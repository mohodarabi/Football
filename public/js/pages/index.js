...
$(document).ready(function () {
  ...
  function updateTotal() {
    ...
    if ($("#carttotal").length) {
      ...
      $("#carttotal").html( $cartTotal+" تومان " );
    }
  }
    ...
    if ($("#cartItems").length) {
      ...
      if (html === "") {
        html =
          ...
          "<h5>با عرض پوزش ، شما هیچ کالایی در سبد خرید ندارید.</h5>" +
          "</div></div></div>";
      }
      $("#cartItems").html(html);
  ...
  if ($(".match_timing").length) {
    $(".match_timing").countdown("1394/10/31", function (event) {
      $(this).html(
        event.strftime(
          '<ul class="counter-wrap ">' +
            "<li ><span>%D</span>روز ها</li>" +
            "<li ><span>%H</span>ساعات</li>" +
            "<li> <span>%M</span>دقایق</li>" +
            "<li ><span>%S</span>ثانیه ها</li>" +
            "</ul>"
        )
      );
    });
